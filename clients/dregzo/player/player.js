...
function playersetup () {
 /*global $*/
    $.getJSON('http://kaboomen.de:8081/extended/'+rev,jsonisthere);
}
...
function jsonisthere (json) {
    ...
    for (var key in json.men) {
     if (document.getElementById('player_'+key) == null) {
      document.getElementById('playground').innerHTML += '<div class="player" id="player_'+key+'"></div>';
     ...
     if (document.getElementById('playername_'+key) == null) {
      document.getElementById('playground').innerHTML += '<div class="playername" id="playername_'+key+'"></div>';
     }
